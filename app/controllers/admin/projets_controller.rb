    ...
    def index
      ...
      begin
        if organisme.ptf_id != nil
          @projets = Projet.where(bailleur_id: current_user.id).or(Projet.where(ptf_id: organisme.ptf_id)).order(id: :desc).page(params[:page]).per(9)
        elsif organisme.iga_id != nil
          @projets = Projet.joins(:igaprojets).where(igaprojets: { iga_id: organisme.iga_id }).or(Projet.where(bailleur_id: current_user.id)).order(id: :desc).page(params[:page]).per(9)
        else
          @projets = []
        end
      rescue
